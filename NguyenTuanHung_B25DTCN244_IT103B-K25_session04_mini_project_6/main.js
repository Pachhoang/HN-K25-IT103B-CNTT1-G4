...
const clearForm = () => {
  ...
  targetId = null;
  elTitle.textContent = "Quản Lý Nhân Viên";
  btnSave.textContent = "Thêm Nhân Viên";
  btnExit.classList.add("hidden");
};

const syncStats = (data) => {
  countTop.textContent = `${data.length} nhân viên`;
  countBot.textContent = `Tổng số nhân viên: ${data.length}`;
};

const displayTable = (data) => {
  listBody.innerHTML = "";
  if (data.length === 0) {
    listBody.innerHTML = `<tr><td colspan="6" style="text-align:center">Chưa có nhân viên nào</td></tr>`;
  } else {
    data.forEach((item) => {
...
const checkValid = () => {
  ...
  if (!fName.value.trim()) {
    msgError.name.textContent = "Tên trống";
    status = false;
  }
  if (!validMail(fMail.value.trim())) {
    msgError.mail.textContent = "Email sai";
    status = false;
  }
  if (!fDate.value) {
    msgError.date.textContent = "Ngày trống";
    status = false;
  }
  if (!fRole.value.trim()) {
    msgError.role.textContent = "Chức vụ trống";
    status = false;
  }
...
empForm.onsubmit = (event) => {
  ...
  if (targetId === null) {
    ...
    staffList = staffList.map((obj) =>
      obj.id === targetId ? { id: targetId, ...info } : obj,
    );
  }
